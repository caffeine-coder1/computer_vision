...
from model import Discriminator, Creator
...
disc = Discriminator(img_size=IMG_SIZE).to(work_device)
gen = Creator(Z_DIM, IMG_SIZE)
...
for epoch in range(EPOCHS):
    ...
    for batch_idx, (real, _) in enumerate(loader):
        ...
        fixed_noise = torch.rand(real.shape[0], Z_DIM).to(work_device)
        # ~~~~~~~~~~~~~~~~~~~ discriminator loop ~~~~~~~~~~~~~~~~~~~ #

        fake = gen(fixed_noise).view(-1, CHANNELS,
                                     H, W)  # dim of (N,1,28,28)
        # ~~~~~~~~~~~~~~~~~~~ forward ~~~~~~~~~~~~~~~~~~~ #
