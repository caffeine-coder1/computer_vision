...
def check_type(obj, types):
    """checks whether the given object's type is in the types list or not.

        Args:
            obj: python object
            types: list of types you want to check the obj against
        returns true if the obj type is in the list.
    """
    known_type = False
    for i in types:
        if isinstance(obj, i):
