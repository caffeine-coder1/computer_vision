...
def check_path(p):
    """Returns a Boolean value. True if file/folder exists.

        Args:
            p: path to check.
    """
    assert isinstance(p, str), f"expecting String for path, got {type(p)}"

    return Path(p).exists()


def create_path(p, create_parents=True):
    """Returns the path as a String. creates parents if not present Already.

        Args:
            p: path to create/return.
            create_parents: Boolean value for parents.
                                True: Creates
    """

    assert isinstance(p, str), f"expecting String for p got {type(p)}"
    ...
    assert not D.exists(), (f"{str(D)} this file structure doesn't exists" +
                            " and create_parents is set to False")
...
def setFileLogger(loggerName, mode='w'):
    ...
    # file handler
    handler = logging.FileHandler(str(logFilePath), mode)
    ...
    """Creates a logger object with given name.
        ...
        Args:
                logger_name: str object, suggested usesage:
                             set_logger(__name__).
                mode: file writing mode. w for overwrite a for append.
    """
    logger = logging.getLogger(logger_name)
    logger.setLevel(logging.DEBUG)
    setConsoleLogger(logger_name)
    setFileLogger(logger_name, mode)
    return logger
...
def save_model(model, current_acc, lowest_acc, D='weights/'):
    """Save the model to given directory. saves both last.pt and best.pt.

        Args:
            model: The model that needs to be saved.(nn.Module object)

            current_acc: accuracy at current state.(float object)

            lowest_acc: lowest accuracy seen till now. (float object)
...
def create_confusion_matrix(y, y_hat, n_classes, figsize=7):
    ...
    cm = ConfusionMatrixDisplay(cm, display_labels=range(n_classes))
